...
var ConversationResponse = (function() {
 ...
 //   UpdateresponseHandler('');
    ...
    responseFunctions = {
      turn_on: {
        appliance: {
          ...
          AirFreshener: function() { Panel.af('af'); },      	
       ...
       af_connected: {
        appliance: {
           		 	 ...
           		 	 AirFreshener: function() {
           		 	 		if (myconnected == "online") 
           		 	 		Panel.af('afonline'); 
           		 	 		else
           		 	 		Panel.af('afoffline'); 
           		 	 		}   
       ...
       af_led: {
        appliance: {
        	    ...
        	    AirFreshener: function() { Panel.af('afledon'); }    
    	}},
       af_button: {
        appliance: {
        	    AirFreshener: function() { Panel.af('afbutton'); }  
    	}},
    	    	
      blinker_on: {
        appliance: {
      	  Blinker: function() { Panel.Blinker('lo'); }
      		},
        
        direction: function(value) { Panel.BlinkerOn(value); }
      },
      turn_off: {
        appliance: {
          lights: function() { Animations.lightsOff(); },
          wipers: function() { Animations.wipersOff(); },
          Blinker: function() { Panel.meOFF(''); }
        }
      },
      turn_up: {
        appliance: {
  ...
  function UpdateresponseHandler(data) {
  	    data.output.text = ['Just Checking updates '];
...
var invalidMultipleDirEntities = true;
      ...
      case 'blinker_on':
        entities.forEach(function(currentEntity) {
          ...
          console.log("CR = entityType = "+entityType);
          if (entityType === 'genre') {
            invalidMultipleEntities = false;
            genreFound = currentEntity;
          }
         if (entityType === 'direction') {
            invalidMultipleDirEntities = false;
            directionFound = currentEntity;
          }
        });
        break;
      default:
    ...
    if  (!invalidMultipleEntities) {
      ...
      callResponseFunction(primaryIntent, primaryEntity);
        ...
        // Otherwise, just take the first one (or the genre if one was found) and
    // look for the correct function to run
    if  (!invalidMultipleDirEntities) {
      var primaryEntity = (directionFound || entities[0]);
      callResponseFunction(primaryIntent, primaryEntity);
    }
  }
