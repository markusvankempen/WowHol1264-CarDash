...
var Panel = (function() {
 ...
 // Rotate the fan in the svgGroup at the speed specified by level
  function animateAF(level, svgGroup) {
    // Find the fan in the DOM and get its initial coordinates
    var fan = Snap.select(idSelectors.fan);
    var bbox = fan.getBBox();
    ...
    var rotateAnim = Snap.animate(0, 100, function(val) {
      ...
      var localMat = fan.transform().localMatrix;
      fan.transform( localMat.rotate(speed, bbox.cx, bbox.cy) );
    }, 5000, mina.linear, function() {}, frameSkipRate);
    currentAnimations.push(rotateAnim);
  }
  
   // BlinkerOn
  function BlinkerOn(direction) {
    ...
    function next(svgFragment, svgGroup) {
      ...
      if (genreText) {
        genreText.textContent = direction.toUpperCase();
      }
    }
	loadSvg('blinker' + direction, next);
  ...
  // Turn on Air-freshner //Blinker,
  function af(level) {
    loadSvg(level, function(svgFragment, svgGroup) {
      animateAF(level, svgGroup);
    });
  }
  // Turn on //Blinker,
  function Blinker(level) {
  	if (level == "off")
  	{
  		meOFF();
  	}else{	 
	loadSvg('blinker', function(svgFragment, svgGroup) {
      animateAF(level, svgGroup);
    });
	}
  }
